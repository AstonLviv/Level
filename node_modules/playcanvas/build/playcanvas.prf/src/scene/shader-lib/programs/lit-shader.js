...
class LitShader {
	...
	_fsGetShadowPassCode() {
		...
		const isVsm = (_shadowInfo$vsm = shadowInfo == null ? void 0 : shadowInfo.vsm) != null ? _shadowInfo$vsm : false;
		let code = this._fsGetBeginCode();
	...
	_fsGetLitPassCode() {
		...
		if (options.clusteredLightingEnabled && this.lighting) {
			...
			if (options.clusteredLightingShadowsEnabled && !options.noShadow) {
				...
				const shadowTypeName = (_shadowTypeInfo$get = shadowTypeInfo.get(options.clusteredLightingShadowType)) == null ? void 0 : _shadowTypeInfo$get.name;
				const clusteredSampleType = shadowTypeName.substring(0, 4);
		...
		if (this.lighting || this.reflections) {
			...
			for (let i = 0; i < options.lights.length; i++) {
				...
				if (light.castShadows && !options.noShadow) {
					...
					const vsmShadows = shadowInfo == null ? void 0 : shadowInfo.vsm;
					const pcfShadows = shadowInfo == null ? void 0 : shadowInfo.pcf;

...
class RenderPassCompose extends RenderPassShaderQuad {
	...
	frameUpdate() {
		...
		if (this._shaderDirty) {
			...
			const key = `${this.toneMapping}` + `-${this.bloomTexture ? 'bloom' : 'nobloom'}` + `-${this.ssaoTexture ? 'ssao' : 'nossao'}` + `-${this.gradingEnabled ? 'grading' : 'nograding'}` + `-${this.vignetteEnabled ? 'vignette' : 'novignette'}` + `-${this.fringingEnabled ? 'fringing' : 'nofringing'}` + `-${this.taaEnabled ? 'taa' : 'notaa'}` + `-${this.isSharpnessEnabled ? 'cas' : 'nocas'}` + `-${this._srgb ? 'srgb' : 'linear'}` + `-${(_this$_debug = this._debug) != null ? _this$_debug : ''}`;
			if (this._key !== key) {
				this._key = key;
